...
void TREEinsert(Tree tree, Item it){
    ...
    if(tree->balanceStrategy == NO_REBALANCE){
		...
		tree->root = insert(tree->root,it);
		if(tree->root->size % (100) == 1){
		    tree->root = balance(tree->root);
		}    
	}else if(tree->balanceStrategy == REBALANCE_1000){
		tree->root = insert(tree->root,it);
		if(tree->root->size % (1000) == 1){
		    tree->root = balance(tree->root);
		}    
...
static link rotLeft (link currentTree) {
	...
	currentTree->size = 1;
	if(currentTree->left != emptyTree){
		currentTree->size += currentTree->left->size;
	}
	if(currentTree->right != emptyTree){
		currentTree->size += currentTree->right->size;
	}
	
	rotLTree->size = 1;
	if(rotLTree->left != emptyTree){
		rotLTree->size += rotLTree->left->size;
	}
	if(rotLTree->right != emptyTree){
		rotLTree->size += rotLTree->right->size;
	}    
	return rotLTree;
}
...
static link rotRight (link currentTree) {
	...
	rotRTree->size = 1;
	if(rotRTree->left != emptyTree){
		rotRTree->size += rotRTree->left->size;
	}
	if(rotRTree->right != emptyTree){
		rotRTree->size += rotRTree->right->size;
	}    	
	return rotRTree;
}
...
/*
void TREEWhiteBoxTest(Tree t){
	Tree newTree = TREEinit()
}*/
